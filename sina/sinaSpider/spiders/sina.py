...
class SinaSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        # 扩展后的 lid 频道列表
        channelList = [str(i) for i in range(2500, 2700)]
        channelList.extend(["2669", "2504", "2505", "2506", "2507"])
    ...
    def parseComments(self, response):
        ...
        for comment in cmtList:
            ...
            if queryList:
                ...
                continue
            # 注意：此处使用的 cookie 可能已过期，如果不抓取评论/用户可忽略
    ...
    def parseUser(self, response):
        ...
        tmp = "".join(selector.xpath("//div[@class='ut']//span[@class='ctt'][1]/text()").extract()).strip().split("\xa0")
        nickName = tmp[0]
        gender = tmp[1].split("/")[0] if len(tmp) > 1 else "未知"
        introduce = "".join(selector.xpath("//div[@class='ut']//span[@class='ctt'][2]/text()").extract()).strip()
